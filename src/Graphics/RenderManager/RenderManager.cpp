...
void RenderManager::GenerateGBuffer()
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, static_cast<GLsizei>(width), static_cast<GLsizei>(height), 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
...
void RenderManager::DirLightPass()
{
	...
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, gPosition);
	differedDirShadingProgram->SetUniform("gPosition", 0);
	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, gNormal);
	differedDirShadingProgram->SetUniform("gNormal", 1);
	glActiveTexture(GL_TEXTURE2);
	glBindTexture(GL_TEXTURE_2D, gDiffuse);
	differedDirShadingProgram->SetUniform("gDiffuse", 2);

	//pass the shadow map textures
	for (int i = 3; i < cascade_levels + 3; i++)
	{
		unsigned idx = i - 3;
		glActiveTexture(GL_TEXTURE0 + i);
		glBindTexture(GL_TEXTURE_2D, shadowmapsText[idx]);
		differedDirShadingProgram->SetUniform("shadowMaps[" + std::to_string(i - 3) + "]", i);
	}
...
void RenderManager::UpdateLightsCount()
{
	...
	{
		glm::vec3 pos{ glm::linearRand(-120, 120),glm::linearRand(0, 130),glm::linearRand(-50, 50)};
		glm::vec3 color = color_struct.colors[glm::linearRand(0, 6)];
		CreateLight(Light::LIGTH_TYPE::POINT, pos, glm::vec3(1), glm::vec3(0), color, lights_radius);
	}
...
void RenderManager::UpdateLightsMovement()
{
	...
	{
		//change direction
		if (glm::abs(light->mPosition.x) > 120 || light->mPosition.y > 130 || light->mPosition.y < 0 || glm::abs(light->mPosition.z) > 50)
